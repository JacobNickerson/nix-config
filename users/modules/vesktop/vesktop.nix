{ config, lib, ... }:
let
	cfg = config.myUserModules.vesktop;
in
{
	options.myUserModules.vesktop = {
		enable = lib.mkEnableOption "Vesktop with custom clearvision fork";
	};
	...
	config = lib.mkIf cfg.enable {
		programs.vesktop = {
			...
			vencord.themes = {
				modified-clearvision = builtins.readFile ./modified-clearvision.css;
			};
			vencord.settings = {
				autoUpdate = true;
